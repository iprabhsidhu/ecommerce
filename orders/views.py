...
def payments(request):
    ...
    # reducing quantity from product stocks
    ...
    # sending reciept on email
    mail_subject = 'Payment Successfull'
    message = render_to_string('orders/reciept.html', {
        'user': request.user,
        'order': order,
...
def send_admin_email(context):
    ...
    message = render_to_string('orders/admin_orders_mail.html', context)
    to_email = 'prabhdeepsidhu310@gmail.com'
    send_email = EmailMessage(mail_subject, message, to=[to_email])
    send_email.content_subtype = "html"
    send_email.send()